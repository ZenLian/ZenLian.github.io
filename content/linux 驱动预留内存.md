...
这里 `mem=512M` 会让 linux 认为系统的内存本身就只有 512M，那么剩下的 512M 物理内存你就可以随意使用了。

也可以使用 `memmap` 参数：

```
memmap=512M@0x40000000,512M$0x60000000
```

- `512M@0x40000000` 表示系统使用从 `0x40000000` 开始的 512M 内存
- `512M@0x60000000` 表示系统预留从 `0x60000000` 开始的 512M 内存

`memmap` 参数更加灵活，可以同时指定多块内存，但这个参数在你的板子或低版本的 linux 内核上不一定会生效。

## 方法二：设备树预留内存

通过设备树预留内存比在启动参数中直接指定内存更加符合驱动开发的规则，但用起来稍微复杂一些。
